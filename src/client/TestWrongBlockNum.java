...
public class TestWrongBlockNum {
	...
	private static boolean receive_DATA_send_ACK(String requestedFile, DatagramSocket sendSocket) throws Exception
	{
		...
		boolean testedWrongBlockNumber = false;
		while(hasMoreData)
		{
			blockNumber++;
			...
			// Sends wrong block number to parseAndWriteData once (this will cause receivedData to be false)
			if(!testedWrongBlockNumber)
			{
				receivedData = parseAndWriteData(buffer, (short)(blockNumber + 8), requestedFile, inPacket.getLength());
			} else 
			{
			...
			// Sends ack with wrong block number once, so that the server should resend the first package again
			if(receivedData)
			{
				sendAck(sendSocket, blockNumber);
			} 
			else if(!testedWrongBlockNumber) 
			{
				sendAck(sendSocket, (short) (blockNumber + 8));
			}
	
			if(!testedWrongBlockNumber) {
				hasMoreData = true;
				testedWrongBlockNumber = true;
				blockNumber--;
			}
			else if(inPacket.getLength()<512)
			{
				hasMoreData = false;
			}
			else
