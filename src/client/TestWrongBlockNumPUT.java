...
public class TestWrongBlockNumPUT
{
	...
	private static boolean write_DATA_receive_ACK(String requestedFile, DatagramSocket sendSocket) throws Exception
	{
		...
		while(remainingFileBytes >= 0)
		{
			...
			if (buffer[3] == 3)   	// START THE faulty implementation to see how server resonse
			{
				buffer[3] = 2;
				datagramSocket.send(outputPacket);
				byte[] tempArray = new byte[BUFSIZE];
				DatagramPacket temp = new DatagramPacket(tempArray, 0, tempArray.length);
				datagramSocket.receive(temp);
				System.out.println("After faulty Blocknumber sent, expect 4,2: actual: " + tempArray[1] + "," + tempArray[3]);
